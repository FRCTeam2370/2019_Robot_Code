...
public class LEDs extends Subsystem {
  ...
  public void sendLightOption() {
    if(RobotState.isAutonomous()){ // if in auto
      i2c.transaction(delay, delay.length, receivable, receivable.length); //  do cool fireworks!
      ...
      if(Robot.kWrist.getWristPosition() > 150){ // if hatch manipulator open
  ...
  public void doLotsOfFireworks(){
    i2c.transaction(fireworks, fireworks.length, receivable, receivable.length);
  }
